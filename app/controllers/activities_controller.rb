...
before_action :set_options_for_activity_type, only: [:new, :edit]
  ...
  def report
    ...
    if params[:q] # && params[:date_to]
      @lobbyist_activities = current_lobbyist.activities.activity_search(params[:q]) #, params[:date_to])
    else
      @lobbyist_activities = current_lobbyist.activities.all.reverse
    end 
  end
  ...
  private
	...
	def activity_params
  	params.require(:activity).permit(:activity_type, :date, :start, :end, :from_phone, :to_phone, :subject, attendees_attributes: [:id, :activity_id, :name, :office, :_destroy])
	end
